...
OPENJEMA_CHROMIUM_PATCHED_MARK_FILE="${WORKDIR}/openjema_chromium_patched"
mark_patched() {
  touch "$OPENJEMA_CHROMIUM_PATCHED_MARK_FILE"
}
mark_reverted() {
  rm -f "$OPENJEMA_CHROMIUM_PATCHED_MARK_FILE"
}

unpatches_openjema() {
  if [[ ! -f "$OPENJEMA_CHROMIUM_PATCHED_MARK_FILE" ]]; then
    einfo "Not patched, may reverted already"
    return
  fi
  einfo "Revert openjema patches.."
  pushd "${CHROME_ROOT}"/src > /dev/null || die "Cannot chdir to ${CHROME_ROOT}/src"
  find_openjema_patches | sort -r | while read -r p; do
...
unpatches_when_aborted() {
  trap 'unpatches_openjema; trap - SIGINT SIGTERM SIGQUIT ERR' SIGINT SIGTERM SIGQUIT ERR # avoid unpatches multiple times
}

cros_pre_src_prepare_patches() {
  einfo "Enter openjema patches.."
  pushd "${CHROME_ROOT}"/src > /dev/null || die "Cannot chdir to ${CHROME_ROOT}/src"
  find_openjema_patches | while read -r p; do
    einfo "Apply:${p}"
    patch -p1 -f -s -g0 --no-backup-if-mismatch -r - < "${p}" || { unpatches_openjema; die; }
  done
  popd || die "Cannot popd from ${CHROME_ROOT}/src"
  mark_patched
}
