...
for overlay in ${BOARD_OVERLAY}; do
  version_config=${overlay}/metadata/jemaos_version.txt
done

base_meta=$(dirname ${BASH_SOURCE[0]})/../metadata
base_version_config=${base_meta}/jemaos_version.txt
...
get_base_jemaos_release_version() {
  get_base_data ${release_tag}
}

get_jemaos_release_version() {
  local default=$(get_base_jemaos_release_version)
